    ...
    <template v-else>
      <div class="common-table-container">
        <div class="common-title-container">
          <el-page-header class="page-header" @back="$router.go(-1)" content="编辑视频"></el-page-header>
        </div>
...
<script>
  ...
  export default {
    ...
    created() {
      this.videoUrlId = this.$route.params.id
      this.fetchData()
    },
    methods: {
      fetchData() {
        this.$post(api.selectVideoUrlById, {
          videoUrlId: this.videoUrlId
        }).then(res => {
          if (res.code === ERR_OK) {
            this.form = res.data
